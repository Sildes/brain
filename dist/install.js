...
function getMarkerRegex(commentStyle) {
    if (commentStyle === 'hash') {
        return new RegExp(`#\\s*===\\s*${MARKER_START}\\s*===[\\s\\S]*?#\\s*===\\s*${MARKER_END}\\s*===`, 'g');
    }
    return new RegExp(`<!--\\s*${MARKER_START}[\\s\\S]*?${MARKER_END}\\s*-->`, 'g');
}
function wrapWithMarkers(content, commentStyle) {
    if (commentStyle === 'hash') {
        return `# === ${MARKER_START} ===
${content}
# === ${MARKER_END} ===`;
...
- Tests: php bin/phpunit`,
        generatePrompt: (promptPath, brainPath) => `Read ${promptPath} and generate the business context for this project.
...
async function installForIde(dir, ideKey, brainPath) {
...
function printGeneratePrompt(ideKey, promptPath, brainPath) {
    const config = IDE_CONFIGS[ideKey];
    if (!config)
        return;
...
export async function installIde(options) {
