import type { Adapter, AdapterMatch, BrainData, Module, Route, Command, Conventions, KeyFile, QuickFindMapping } from "../types.ts";
...
async function runCommand(cmd: string, cwd: string): Promise<{ stdout: string } | null> {
  try {
...
function inferModulesFromApp(appFiles: string[]): Module[] {
  ...
  for (const file of appFiles) {
    ...
    if (skipDirs.includes(topDir)) continue;
...
async function extractRoutesViaArtisan(dir: string): Promise<Route[]> {
...
async function extractCommandsFromApp(dir: string, appFiles: string[]): Promise<Command[]> {
  const commands: Command[] = [];
  const commandFiles = appFiles.filter((f) =>
    f.includes("/Commands/") && f.endsWith(".php")
  );
...
async function identifyKeyFiles(dir: string, appFiles: string[]): Promise<KeyFile[]> {
...
export const laravelAdapter: Adapter = {
  name: "laravel",
  priority: 90,
