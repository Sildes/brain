import type { Adapter, AdapterMatch, BrainData, Module, Route, Command, KeyFile, Conventions, QuickFindMapping } from "../types.ts";
...
async function runCommand(cmd: string, cwd: string): Promise<{ stdout: string; stderr: string } | null> {
  try {
...
function inferModulesFromSrc(srcFiles: string[]): Module[] {
...
async function extractRoutesViaCli(dir: string): Promise<Route[]> {
  ...
  try {
    ...
    return Object.entries(routes).map(([name, info]: [string, any]) => ({
      name,
...
function inferModuleFromController(controller: string | undefined): string | undefined {
...
async function extractCommands(dir: string, srcFiles: string[]): Promise<Command[]> {
...
async function extractKeyFiles(dir: string, srcFiles: string[]): Promise<KeyFile[]> {
...
function extractConventions(composer: ComposerJson): Conventions {
  ...
  conventions.navigation.push(
    { description: "List all routes", command: "php bin/console debug:router" },
    { description: "List all services", command: "php bin/console debug:container" },
    { description: "Run tests", command: "php bin/phpunit" }
  );

  return conventions;
}

function buildQuickFind(): QuickFindMapping[] {
  return [
...
export const symfonyAdapter: Adapter = {
  name: "symfony",
  priority: 100,

  async detect(dir: string): Promise<AdapterMatch> {
    const composerPath = path.join(dir, "composer.json");
    const composer = await readJson<ComposerJson>(composerPath, {});
  ...
  async extract(dir: string): Promise<BrainData> {
    ...
    // Read composer.json
    const composer = await readJson<ComposerJson>(path.join(dir, "composer.json"), {});
