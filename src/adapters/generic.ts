import type { Adapter, AdapterMatch, BrainData, Module, Conventions, KeyFile, QuickFindMapping } from "../types.ts";
...
function inferModulesFromSrc(srcFiles: string[]): Module[] {
  ...
  for (const file of srcFiles) {
    ...
    if (skipDirs.includes(topDir.toLowerCase())) continue;
...
async function identifyKeyFiles(dir: string, files: string[]): Promise<KeyFile[]> {
  ...
  const srcFiles = files.filter((f) =>
    /\.(ts|tsx|js|jsx|py|php|go|rs|java)$/i.test(f) &&
...
function inferConventions(): Conventions {
  return {
    standards: [],
    notes: [
...
export const genericAdapter: Adapter = {
  ...
  async detect(_dir: string): Promise<AdapterMatch> {
  ...
  async extract(dir: string): Promise<BrainData> {
    ...
    const srcFiles = allFiles.filter((f) =>
      f.startsWith("src/") || f.startsWith("app/") || f.startsWith("lib/")
    );
