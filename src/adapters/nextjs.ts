import type { Adapter, AdapterMatch, BrainData, Module, Route, Conventions, KeyFile } from "../types.ts";
...
function extractRoutesFromAppDir(appFiles: string[]): Route[] {
  const routes: Route[] = [];
...
function extractRoutesFromPagesDir(pagesFiles: string[]): Route[] {
  const routes: Route[] = [];
  ...
  for (const file of pagesFiles) {
    ...
    if (/pages\/_/.test(normalized)) continue;
...
function inferModulesFromApp(appFiles: string[]): Module[] {
  ...
  for (const file of appFiles) {
    ...
    if (appMatch) {
      ...
      if (topDir.startsWith("(") || ["api", "(auth)", "(dashboard)"].includes(topDir)) continue;
...
export const nextjsAdapter: Adapter = {
  ...
  async detect(dir: string): Promise<AdapterMatch> {
    const packageJson = await readJson<PackageJson>(path.join(dir, "package.json"), {});
